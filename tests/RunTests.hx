	...
	public function new() {		
		describe('rules', function () {
			for (test in tests) {
				var path = test[0];
				var opts = test[1];
				var tokens: Array<Dynamic> = test[2];
				var matchCases = test[3];
				var compileCases: Array<Dynamic> = test[4];

				var keys: Array<Key> = cast tokens.filter(function (token)
					return Std.is(token, String)
				);
				...
				describe(path, function () {
					...
					it('should parse', function () {
						...
						for (i in 0 ... parsed.length) {
							switch parsed[i] {
								case Static(token): token.should.be(tokens[i]);
								case Capture(token):
									structEq(token, tokens[i]);
							}
						}
					});
					...
					describe('compile', function () {
						...
						for (io in compileCases) {
							...
							if (output != null) {
								if (input == null) input = {};
								it('should compile using ' + input, function () {
									structEq(output, toPath(input));
								});
							} else {
...
/*
	...
	// From: https://github.com/haxetink/tink_web/blob/master/tests/DispatchTest.hx
	function structEq<T>(expected:T, found:T) {
		function fail(msg) throw msg;
		
		function assertEquals<T>(a: T, b: T)
			if (a != b)
				fail('Expected '+a+', was '+b);
